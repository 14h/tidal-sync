...
import type { Playlist, Track, StreamUrl } from "./types.js";
...
async function downloadSegments(urls: string[]): Promise<Buffer> {
  const chunks: Buffer[] = [];
...
async function setMetadata(
  ...
  try {
    ...
    console.warn(chalk.yellow(`  Warning: Could not set metadata — ${(err as Error).message}`));
  }
}
